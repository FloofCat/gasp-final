...
class SuffixLLM:
    ...
    def check_if_trained(self):
        ...
        if not os.path.exists(f"./gasp-smoothllm-2/models/{self.model_name}_finetuned"):
            return False
        return True
    ...
    def train_model(self):
        ...
        self.trainer.train()
        self.model.save_pretrained(f"./gasp-smoothllm-2/models/{self.model_name}_finetuned")
        self.tokenizer.save_pretrained(f"./gasp-smoothllm-2/models/{self.model_name}_finetuned")
    ...
    def setup_inference(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(f"./gasp-smoothllm-2/models/{self.model_name}_finetuned",
                                                            torch_dtype=torch.float16,
                                                            trust_remote_code=True,
                                                            device_map="auto")
        # .to(self.device) -- Seems like ORPO hates this. OOM at all times.
        self.tokenizer = AutoTokenizer.from_pretrained(f"./gasp-smoothllm-2/models/{self.model_name}_finetuned",
                                                        trust_remote_code=True,
                                                        use_fast=False)
        self.tokenizer.pad_token = self.tokenizer.eos_token
    ...
    def load_orpo_model(self, blackbox_name):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(f"./gasp-smoothllm-2/models/{blackbox_name}_orpo",
                                                            torch_dtype=torch.float16,
                                                            trust_remote_code=True,
                                                            device_map="auto")
        self.tokenizer = AutoTokenizer.from_pretrained(f"./gasp-smoothllm-2/models/{blackbox_name}_orpo",
                                                        trust_remote_code=True,
                                                        use_fast=False)
